...
glmmTMB is not supported by MuMIn, so switching to lmer
```{r}
mod_sens1 <- lmer(ninetyfive ~ 
                       Treatment * prod + I(prod^2)
                     + YrsSinceExclosure+ YrsSinceExclosure:Treatment + 
                    (1|region) +
                    (1|LocalityName),
                    data = dat2,
                  REML = F)

mod_sens2 <- lme4::lmer(ninetyfive ~ Treatment * prod + I(prod^2)
                     + YrsSinceExclosure+ YrsSinceExclosure:Treatment +
                       (1|LocalityName),
                       data = dat2,
                     REML = F)
...
Model averaging and the use of quadratic terms both indicate we should scake and centre predictors
```{r}
stdz_model <- standardize(mod_sens2, standardize.y = FALSE)
summary(stdz_model)
```
...
(cg_cand <- dredge(stdz_model, beta="none", rank = "AICc"))
```
...
```
We exclude models with quadratic terms when the main effect is not in the model, but none such here.

Lets export this as a table for the supplementary
```{r}
cg_cand2X <- as.data.frame(cg_cand2)
names(cg_cand2X) <- c("Intercept",
...
#write.csv(cg_cand2X, "../output/95thModelSet.csv", row.names = F)
```


Average across these three models
```{r}
MA.ests<-model.avg(cg_cand2, revised.var = TRUE)
summary(MA.ests)
```
...
```{r}
(macdf <- data.frame(summary(MA.ests)$coefmat.full[-1,]))
```
...
Then lets male the figure
First, adding empty row
```{r}
macdf4 <- macdf3[-(1:nrow(macdf3)),]
macdf4[1,] <- c(NA, 100, NA, NA, NA, NA, NA, NA, NA) 
...
b1 <- barplot(macdf3[,2],
            ...
            xlim=c(-1,2),
            las=1,
            xlab='Model averaged coefficients',
            cex.axis=0.8,
            cex.lab=0.8)
points(macdf3[,2], #est
       b1,
...
This can be exported but I'll wait and do it later
```{r, eval=F}
...
Then we need to plot canopy height against experimental duration

```{r}

Treatment <- rep(c("Open plot", "Exclosure"), each=50)

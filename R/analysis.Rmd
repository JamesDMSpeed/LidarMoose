...
To avoid three level interaction we divide by duration. The linearity of these relationships have been tested below
```{r}
...
corrgram(dat2[,c("vci", "rumple", "prod", 
...
After careful sensitivity analysis, I decided to drop these two locations. Scripts are moved to the end of the file
...
```{r}
mod_sens1 <- glmmTMB(ninetyfive ~ Treatment * prod + I(prod^2)
                     + YrsSinceExclosure+ YrsSinceExclosure:Treatment + (1|region) + (1|LocalityName),
  data = dat2,  family = gaussian)

mod_sens2 <- glmmTMB(ninetyfive ~ Treatment * prod + I(prod^2)
                     + YrsSinceExclosure+ YrsSinceExclosure:Treatment +  (1|LocalityName),
  data = dat2,  family = gaussian)
AIC(mod_sens1, mod_sens2)
# regions explains nothing, so I'm removing it
#summary(mod_sens2)
library(MuMIn)
(cg_cand <- dredge(mod_sens2, beta="none", rank = "AICc"))
#write.csv(cg_cand, "/home/anders/Documents/lidar ms/canopyGrowth.csv")

# best model is:
cg_best <- glmmTMB(ninetyfive ~ Treatment * YrsSinceExclosure + (1|LocalityName),
  data = dat2,  family = gaussian)
summary(cg_best)
#
...
```{r, eval =F}
sink("/home/anders/Documents/lidar ms/canopyGrowth.csv")
print(summary(cg_best))
sink()
```
...
library(reshape2)
#datCast <- dcast(data=dat2, value.var="canopygrowth", LocalityName~Treatment)
#datCast$Effect <- datCast$Exclosure - datCast$`Open plot` 
#datCast$Treatment <- "Canopy height (95th percentile) growth per year"
#ggplot(data=datCast)+
#  geom_violin(aes(y=Effect, x=Treatment))+
#  coord_flip()

# but I'd like to see the real numbers
...
We can also make these nice violin plots, even though the main plot should show the interaction effect like the plot above does.
```{r}
(cg_viol <- 
   ggplot(data = dat2, aes(x = Treatment, y = canopygrowth))+
...
YrsSinceExclosure <- rep(seq(from = min(dat2$YrsSinceExclosure), 
                             to=max(dat2$YrsSinceExclosure), length.out = 50), 2)
LocalityName <- rep(NA, times=100)

pred <- predict(cg_best, 
                list(
            Treatment         =tr, 
            YrsSinceExclosure =YrsSinceExclosure,
...
# Biomass 
As explained above, we're gonna try a gamme-hurdle model
```{r}
...
```{r}
agb_viol <- 
   ggplot(data = dat2, aes(x = Treatment, y = MgAGBperYearAndHA))+
  geom_violin(fill = "grey")+
  theme_bw()+
  theme(text = element_text(size = 12))+
...
datB <- dcast(data=dat2, value.var="MgAGBperYearAndHA", LocalityName+prod~Treatment)
...
```{r}
cor(dat2$vci, dat2$MgAGBperYearAndHA)
```
...
```{r}
dat2$prod2 <- dat2$prod*dat2$prod
mod_vci <- glmmTMB(vci ~ Treatment * prod + YrsSinceExclosure + YrsSinceExclosure:Treatment+ prod2 +  (1|LocalityName),
  data = dat2,  family = gaussian)
dmod_vci <- dredge(mod_vci)
mod_vci <- glmmTMB(vci ~ Treatment + prod + prod2 + YrsSinceExclosure+ (1|LocalityName),
  data = dat2,  family = gaussian)
summary(mod_vci)
```
...
prod2 <- prod^2
YrsSinceExclosure <- rep(mean(dat2$YrsSinceExclosure), times=100)
LocalityName <- rep(NA, times=100)

pred <- predict(mod_vci, 
                list(
            Treatment         =tr, 
            prod              =prod,
            prod2             =prod2,

...
ggplot(dat, aes(x=Treatment, y=canopygrowth, group=LocalityName))+
  geom_line()+
...
anova(mod2, mod3)
```
The numbers tell us we should keep it.
...
```
Prorbably no interaction with productivityhere, but the 'exclosure line' appear to lie higher.
...
ggplot()+
  ...
  theme(legend.position = 'right',
                             ...
                             legend.text = element_text(size=12))
```
That looks a bot odd. Lets look at it without backtransforming the x-axis:
```{r}
ggplot()+
...
data.frame("mean canopy growth per year" = round(m, 3),
                        "standard error of the mean" = round(s, 3))
```
...
data.frame("mean canopy growth per year" = round(m2, 3),
...
```{r}
library(MuMIn)
dredge(moddiff, beta="none", rank = "AICc")[1:5,]
```
...
```
Thta last line is tricky (https://github.com/tidyverse/ggplot2/issues/2322)

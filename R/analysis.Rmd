...
plot(dat$canopygrowth)
```


The other option would be to add time since clear-cutting as a fixed effect to the models. That would complicate the models obviously, but it would allow fitting non-linear effects of time. Here we impose linearity. Let's see what it looks like for canopy height.
...
Lets check our assumption about linear growth over time:
```{r}
ggplot(data = dat2,
                    aes(x = YrsSinceExclosure, y = ninetyfive))+
  geom_point(aes(colour= Treatment, shape=region))+
   geom_smooth(aes(colour= Treatment),
               method = "loess")+
  labs(y='Canopy height', x='Years since exclosure')+
...
```{r}
mod_sens1 <- glmmTMB(canopygrowth ~ Treatment * prod + I(prod^2) + (1|region) + (1|LocalityName),
  data = dat2,  family = gaussian)

mod_sens2 <- glmmTMB(canopygrowth ~ Treatment * prod + I(prod^2)+(1|LocalityName),
  data = dat2,  family = gaussian)
...
# best model is:
cg_best <- glmmTMB(canopygrowth ~ Treatment +(1|LocalityName),
  data = dat2,  family = gaussian)
summary(cg_best)
#
...
cg_viol <- 
  ...
  labs(y=expression(paste('Canopy height (95th percentile) growth (m year'^'-1', ')')), x='')+
...
ch_viol <- 
  ...
  theme_bw()+
  theme(text = element_text(size = 12))+
  labs(y='Canopy height (95th percentile)')+
  stat_summary(fun.y=mean, geom="point", shape=23, size=2)+
  xlab("")
